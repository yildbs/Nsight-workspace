...
FrameSubstractor& FrameSubstractor::Substract(cv::Mat background,cv::Mat frame)
{
	...
	for(int y=0;y<block_height;y++){
		for(int x=0;x<block_width;x++){
			...
			float sum = 0.;
			for(int i=0;i<this->params.nbins*4;i++){
...
//			printf("x, y = %d, %d : %.5f\n", x, y, sum);
			...
			if(sum >= 1.5){
				int x_start = x * this->params.cell_size;
				int x_end = (x+2) * this->params.cell_size;
				int y_start = y * this->params.cell_size;
				int y_end = (y+2) * this->params.cell_size;
